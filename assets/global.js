...
// Sticky filter bar
const filterBar = document.querySelector('.facets-container, .facets-container-drawer');

if (filterBar) {
  const offsetTop = filterBar.getBoundingClientRect().top + window.scrollY;
  ...
  window.addEventListener('scroll', () => {
    if (window.scrollY > offsetTop) {
      const headerHeight = document.querySelector('sticky-header').getBoundingClientRect().height;
      const headerBottom = document.querySelector('sticky-header').getBoundingClientRect().bottom;
      const topOffset = headerBottom > 0 ? headerBottom : 0;
      ...
      filterBar.style.background = '#fff';
      filterBar.style.padding = '0 4.8rem';
      filterBar.style.transition = 'top 0.1s ease';
    } else {
      placeholder.style.display = 'none';
